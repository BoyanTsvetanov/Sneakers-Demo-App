  ...
  return (
    <section name="Продукти" className='mt-4'>
        ...
        <div className='flex items-center justify-end mr-0'>
            <h1 className='text-2xl font-bebas font-bold border-4 rounded-full p-2'>Продукти</h1>
            <div className='h-1 w-[80%] bg-dark dark:bg-light'></div>
        </div>

        <div className="mt-6 mx-16 max-md:mx-8 rounded-3xl p-1 flex gap-6 max-sm:gap-1.5 bg-transparent border-4">
            ...
            <ul className={clsx("bg-transparent dark:border-light not-dark:bg-light border-2 rounded-3xl p-3 h-min ", filter ? 'absolute block dark:bg-dark' : 'max-sm:hidden')}>
                {sneaker_categories.map((category) => (
                <li key={category.id} className={clsx("mx-8 my-4 pb-4 border-b-2 text-pretty border-light/25 not-dark:border-dark/25 last:border-b-2 dark:hover:border-light not-dark:hover:border-dark transition-all duration-500", filter && 'first:mt-10')}>
