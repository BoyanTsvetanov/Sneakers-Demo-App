...
const Header = () => {
  ...
  const NavLink = ({ title }) => (
    <LinkScroll
      ...
      className="font-montserrat leading-normal font-bold hover:underline z-10 cursor-pointer brightness-200 text-white"
    >
      {title}
    </LinkScroll>
  );
  ...
  return (
    <header
      className={clsx(
        ...
        hasScrolled
          ? "py-2 max-md:py-1 not-dark:bg-dark dark:bg-[hsl(259,77%,25%)]"
          : "py-6 md:py-4 max-md:py-2"
      )}
    >
      <nav className="flex justify-between items-center max-md:justify-between">
        ...
        <div className="relative flex gap-8">
          <ul
            className={clsx(
              "flex justify-center text-light text-xl items-center gap-10 max-lg:opacity-0 max-lg:absolute max-lg:flex-col max-lg:bg-light max-lg:text-4xl max-lg:text-primary-dark max-lg:w-[100%] max-lg:h-screen max-lg:top-0 max-lg:right-0 max-lg:bg-gradient-to-br from-accent-green to-[hsl(259,77%,25%)] max-lg:size-8 max-lg:bor",
          ...
          >
            ...
            <div className="hidden max-lg:block w-full h-full inset-0 absolute bg-[linear-gradient(to_right,rgba(0,0,0,0.5)_2px,transparent_2px),linear-gradient(to_bottom,rgba(0,0,0,0.5)_2px,transparent_2px)] bg-[size:24px_24px] z-0">
              <img
                src="./images/slidebar.png"
                alt="slidebar"
                className="absolute w-full object-contain h-full"
              />
            </div>
          ...
          <div className="flex  p-1 rounded gap-1 items-center">
            <Button title="" icon="./icons/cart.png"></Button>
            <div className="w-0.25 h-[24px] bg-light"></div>
            <Link to="/Profile">
              <Button title="" icon="./icons/user-profile.png"></Button>
            </Link>
          </div>
        </div>
        ...
        >
          <img
            ...
            alt="menu"
            className={clsx("p-1", hasScrolled && "not-dark:bg-light")}
          />
        </button>
      </nav>
    </header>
